...
class rocket_launch:
    ...
    def __init__(self, mission, fuel_mass, engine, launch_angle, time, verbose=False):
        ...
        self.analytical = True # Flag for using analytically calculated orbits
    ...
    def __str__(self):
        ...
        str = str + f"      Planet orbital angle | {self.system.initial_orbital_angles[0]} r\n"
        str = str + f"        Planet initial pos | {self.system.initial_positions[:,0]} r\n"
    ...
    def final_position(self):
        ...
        if(self.verbose):
            print(f'Launch site position {self.launch_site_position} AU at escape time {self.escape_time}')
    ...
    def get_launch_pos(self,t):
        ...
        if(self.verbose):
            print(f'Planet positon at {t} is {pos}')
    ...
    def planet_position_analytical(self, n, t):
        ...
        x,y,v = self.orbit_sim.hoth_analytical_solution(t)
        return np.array([x,y])
    ...
    def plot_orbit(self,n,zoom_to_planet=True):
        ...
        if(self.verbose):
            print(f'Planet position {planet_position} at {self.escape_time}')
            print(f'Analytical pos {self.get_trajectories(n)[:,0]}')
        ...
        if(zoom_to_planet == True):
            scale = 2
            ax.set_xlim([planet_position[0]-self.mission.system.radii[n]*scale*1000/c.AU,planet_position[0]+self.mission.system.radii[n]*scale*1000/c.AU])
            ax.set_ylim([planet_position[1]-self.mission.system.radii[n]*scale*1000/c.AU,planet_position[1]+self.mission.system.radii[n]*scale*1000/c.AU])
        else:
            scale = 1.05
            ax.set_xlim([self.system.semi_major_axes[0]*-scale,self.system.semi_major_axes[0]*scale])
            ax.set_ylim([self.system.semi_major_axes[0]*-scale,self.system.semi_major_axes[0]*scale])

        # Plot the star
        star=plt.Circle((0,0),self.mission.system.star_radius*1000/c.AU,color='y',fill=True)
