...
class TrajectionComputer:
    """The responsibility of this class is to calculate the spacecraft trajectory.

    from a given time, position and initial velocity
    """
    def __init__(self, spacecraft_mass, seed=33382):
        """Initialize the TrajectionComputer."""
        # Configures logging
    ...
    def planet_positions_at_t(self, t):
        """Return planet position at time t."""
    ...
    def planet_position(self, n, t):
        ...
        self.log.info(f'Planet position is {self.planet_positions[:,n,idx+1]} \
                        at index {idx+1} at time {self.times[idx+1]}')
    ...
    def planet_orbital_velocity(self, n, t):
        ...
        self.log.debug(f'Calculated orbital velocity {planet_velocity} = {deltaS}/{deltaT} : \
                        {np.linalg.norm(planet_velocity*c.AU/c.yr)} m/s')

        return planet_velocity

    def animate_orbits(self, t0, t, steps):
        """Plot an animation of the orbits and trajectory."""
        print('animate_orbits')
        fig, ax = plt.subplots()
        ...
        anim.save('orbit_animation.gif', writer='imagemagick', fps=50)
        # plt.show()

    def planet_html_table(self, t):
        """Prints a HTML  table representation of the planets."""
        print('<table><tr><th>Planet</th><th>Masse i sol masser</th>\
                <th>Posisjon i AU</th><th>Hastighet i AU/år</th></tr>')
        for n in range(self.system.number_of_planets):
            print(f'<tr><td>{n}</td><td>{self.system.masses[n]}</td><td>({self.planet_position(n,t)[0]},\
                        {self.planet_position(n,t)[1]})</td><td>({self.planet_orbital_velocity(n,t)[0]},\
                        {self.planet_orbital_velocity(n,t)[1]})')
        print('</table>')
    ...
    def simulate_trajectory(self, initial_time, initial_position, initial_velocity, simulation_time, dt):
        ...
        def f(r, t):
            ...
            for i in range(N):
                ...
                if(math.isnan(ap[i, 0])):
                    ap[i, 0] = 0.0
                if(math.isnan(ap[i, 1])):
                    ap[i, 1] = 0.0
...
if __name__ == '__main__':
    ...
    initial_time = 91*c.day/c.yr
    print(f'Planet 0 position at t={initial_time} [{tcomp.planet_position(0, initial_time)[0]}\
            ,{tcomp.planet_position(0, initial_time)[1]}]')
    initial_position = tcomp.planet_position(0, initial_time)
    initial_velocity = np.array([-0.42020143634159757, 10.371306519368773])

    print('Parameters')
    print(f'Initial time {initial_time}')
    print(f'Initial position {initial_position}')
    print(f'Initial velociy {initial_velocity}')
    # np.array([3.1027160427391958, 9.899270080747108])  # np.array([2.5593576572450947, 8.92172415360237])
    # [3.035884962597734,8.90150577655141]
    # Velocity after launch [-2.654744956763029,7.3589109432903985]
    # [-0.8274927968208055,0.5614763318339263]
    steps = 2000
    days = 120
    duration = 0.48857271841706096/2  # days*c.day/c.yr
    dt = duration/steps
    intermediate_time, position, velocity = tcomp.simulate_trajectory(initial_time, initial_position,
                                                                      initial_velocity, duration, dt)
    ...
    print(f'   Final position [{position[0]},{position[1]}] AU')
